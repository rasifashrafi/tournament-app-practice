...
function setTeams(teamsArray) {
  ...
  for (var i = length; i > (length - 8); i--) {
    ...
    //console.log(random)
    //Add the element to the teamContainer using InnerHTML and 
    //select the element by using the random number to grab from the teamsArray. 
    //Once you are done push the selected element into the temp array and 
    //remove the element from the teamsArray by using the function splice
...
function playGame(round, match, place) {
  ...
  //return the winner
  return rounds[`round${round + 1}`][x][place] = currentPlayers[1]
